...
/**
 * Class Amount2text
 * Russian, Multy Currencies Editions
 */
...
class Amount2text
{
    ...
    {
        $this->Amount = $Amount;
        // TODO проверить что сумма не минусовая
    ...
    private function getTriadeDescription($stage, $morph) {
        switch ($stage) {
            case '4': //Триллионы
                if ($morph == 3) $result = 'триллионов ';
                if ($morph == 2) $result = 'триллиона ';
                if ($morph == 1) $result = 'триллион ';
                break;
            case '3':  //Миллиарды
...
#$a2t->setAmount($ii);
#echo $ii ." ". $a2t->getText(). "<br>";


for ($i = 0; $i <= 100; $i++) {
    $k = $ii + $i;

    echo $k ." ". $a2t->getText($k). "<br>";
}
